...
def mean_class_accuracy(scores, labels):
    ...
    # 确保 scores 是二维数组 (num_samples, num_classes)
    if scores.ndim == 0:
        # 标量情况：转换为二维数组 (1, 1)
        scores = scores.reshape(1, 1)
    elif scores.ndim == 1:
        # 一维数组：转换为二维数组 (1, num_classes)
        scores = scores.reshape(1, -1)
...
def top_k_accuracy(scores, labels, topk=(1, )):
    ...
    # 确保 labels 和 scores 的第一维匹配
    if labels.shape[0] != scores.shape[0]:
        # 如果 labels 是标量且 scores 有多个样本，调整 labels
        if labels.size == 1 and scores.shape[0] > 1:
            labels = np.repeat(labels, scores.shape[0], axis=0)
        else:
            # 否则，截断或填充以匹配
            min_len = min(labels.shape[0], scores.shape[0])
