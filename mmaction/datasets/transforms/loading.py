...
class SampleFrames(BaseTransform):
    ...
    Args:
        ...
        target_fps (optional, int): Convert input videos with arbitrary frame
            ...
            ``None``.
        use_random_offset (bool): Whether to apply random offset to clip
            positions during training. If set to False, clips will be
            evenly distributed without random offset. Defaults to True.
            This only affects training mode.
    """
    ...
    def _get_train_clips(self, num_frames: int,
        ...
        if use_random_offset is True. Otherwise, clips will be evenly
        distributed without random offset.
        ...
        if self.keep_tail_frames:
            ...
            if avg_interval > 0:
                ...
                if self.use_random_offset:
                    ...
                    # Evenly distribute when not using random offset
                    max_offset = num_frames - ori_clip_len + 1
                    clip_offsets = np.linspace(0, max_offset, self.num_clips,
